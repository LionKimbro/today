"""Today combined model: reconfigurable Tk workspace plus Mobile Stacks.

This experiment deliberately has three visible layers:

    Tk interaction -> Mobile Stack -> Mem semantic change -> Disk -> Tk redraw

The `03` layout body remains available here while the old Tk-local event loop
is being replaced by the explicit Tk, Mem, and Disk machines from `05`.
"""
...
# `06` installs its Mobile Stacks entrypoint below.


# ---------------------------------------------------------------------------
# Mobile Stacks nervous system and machine-owned semantic world

memory = {"days": {}, "panels": {}}
# The 06 merge proves Disk ownership and routing with an in-memory stand-in.
# The day-oriented JSON filesystem layout is the next persistence step.
disk_store = {"days": {}}
machines = {
...
def active_machine():
    return machines[current_thread().name]


def active_stack():
    stack = active_machine()["stack"]
    if stack is None:
        raise RuntimeError("this operation requires an active Mobile Stack")
    return stack


def top():
    return active_stack()["frames"][-1]
...
def begin_stack(machine_name, operation):
    if active_machine()["stack"] is not None:
        raise RuntimeError("cannot begin a Mobile Stack while another is active")
    active_machine()["stack"] = {"registers": {}, "frames": [{"machine": machine_name, "op": operation}]}
...
def submit_stack():
    ...
    if not stack["frames"]:
        source["stack"] = None
        return
    target_name = stack["frames"][-1]["machine"]
    source["stack"] = None
    if target_name == current_thread().name:
        machines[target_name]["run-queue"].append(stack)
        if target_name == "tk":
            schedule_tk_pump()
    elif target_name == "tk":
        ...
        machines[target_name]["in-queue"].put(stack)


def admit_stacks():
    while True:
        try:
            stack = active_machine()["in-queue"].get_nowait()
        except Empty:
            return
...
def run_one_stack():
    ...
    if stack is None:
        return None
    machine["stack"] = stack
    if top()["machine"] != current_thread().name:
        submit_stack()
        return True
    machine["handler"]()
    submit_stack()
...
def run_worker_machine():
    while True:
        ...
        if result is None:
            return
        if result is False:
            active_machine()["run-queue"].append(active_machine()["in-queue"].get())


def schedule_tk_pump():
    if g["root"] is not None and not g.get("tk-pump-scheduled"):
...
def make_memory_panel(day_text, panel_type, text=None):
    panel_id = f"panel-{uuid.uuid4().hex[:10]}"
    memory["panels"][panel_id] = {
        "id": panel_id, "type": panel_type, "day": day_text,
        "data": {"text": text or "", "items": [{"text": "New item", "done": False}]},
    }
    return panel_id


def make_memory_day(day_text):
    tab = {"tab_id": f"tab-{uuid.uuid4().hex[:10]}", "title": "main", "rows": [make_row(2), make_row(1)]}
    model = {"date": day_text, "tabs": [tab], "selected_tab_id": tab["tab_id"], "panel_at": {}, "special_panel_slots": {}}
    model["special_panel_slots"]["day_orientation"] = make_memory_panel(day_text, DAY_ORIENTATION, f"Orientation for {day_text}")
    ensure_memory_positions(model, day_text)
    return model


def ensure_memory_positions(model, day_text):
    valid = set(iter_positions(model))
    for position in valid:
        if position not in model["panel_at"]:
            panel_type = JOURNAL if model["panel_at"] else EMPTY
            model["panel_at"][position] = make_memory_panel(day_text, panel_type)
    for position in list(model["panel_at"]):
...
def make_day_bundle(day_text):
    model = deepcopy(memory["days"][day_text])
    return {"day": day_text, "layout": model, "panels": {panel_id: deepcopy(panel) for panel_id, panel in memory["panels"].items() if panel["day"] == day_text}}


def install_day_bundle(day_text, bundle):
    memory["days"][day_text] = deepcopy(bundle["layout"])
    for panel_id, panel in list(memory["panels"].items()):
        if panel["day"] == day_text:
            del memory["panels"][panel_id]
    memory["panels"].update(deepcopy(bundle["panels"]))


def apply_memory_event(day_text, event):
    before_bundle = make_day_bundle(day_text)
    model = memory["days"][day_text]
    event_type = event["type"]
    if event_type in (TOGGLE_TODO, EDIT_JOURNAL, EDIT_ORIENTATION):
        panel_id = event.get("panel_id") or model["special_panel_slots"]["day_orientation"]
        panel = memory["panels"][panel_id]
        if event_type == TOGGLE_TODO:
            panel["data"]["items"][0]["done"] = not panel["data"]["items"][0]["done"]
        else:
            panel["data"]["text"] = event["text"]
        return before_bundle != make_day_bundle(day_text)
    next_model, _effects = reduce_model_with_memory_panels(model, event, day_text)
    memory["days"][day_text] = next_model
    return before_bundle != make_day_bundle(day_text)


def reduce_model_with_memory_panels(model, event, day_text):
    next_model = deepcopy(model)
    event_type = event["type"]
    if event_type == SELECT_TAB:
        if find_tab(next_model, event["tab_id"]):
            next_model["selected_tab_id"] = event["tab_id"]
    elif event_type == SET_PANE_COUNT:
        tab = find_tab(next_model, event["tab_id"])
        row_number = event["row_number"]
        if tab and 1 <= row_number <= len(tab["rows"]):
            ...
            row["pane_count"] = pane_count
            row["sash_proportions"] = [(index + 1) / pane_count for index in range(pane_count - 1)]
            ensure_memory_positions(next_model, day_text)
    elif event_type == ADD_ROW:
        tab = find_tab(next_model, event["tab_id"])
        if tab:
            tab["rows"].append(make_row(1))
        ...
        tab = {"tab_id": f"tab-{uuid.uuid4().hex[:10]}", "title": next_tab_title(next_model), "rows": [make_row(2), make_row(1)]}
        next_model["tabs"].append(tab)
        next_model["selected_tab_id"] = tab["tab_id"]
        ensure_memory_positions(next_model, day_text)
    elif event_type == RENAME_TAB:
        tab = find_tab(next_model, event["tab_id"])
        if tab and event["title"].strip():
            tab["title"] = event["title"].strip()
    elif event_type == DELETE_TAB:
        if len(next_model["tabs"]) > 1:
            index = next((i for i, tab in enumerate(next_model["tabs"]) if tab["tab_id"] == event["tab_id"]), None)
            if index is not None:
                next_model["tabs"].pop(index)
                if next_model["selected_tab_id"] == event["tab_id"]:
                    next_model["selected_tab_id"] = next_model["tabs"][min(index, len(next_model["tabs"]) - 1)]["tab_id"]
                ensure_memory_positions(next_model, day_text)
    elif event_type == SET_SASH_PROPORTIONS:
        tab = find_tab(next_model, event["tab_id"])
        row_number = event["row_number"]
        ...
        position = tuple(event["position"])
        if position in next_model["panel_at"]:
            next_model["panel_at"][position] = make_memory_panel(day_text, event["panel_type"])
    return next_model, []


def render_day_bundle():
    bundle = get_register("day-bundle")
    if bundle["day"] != g["selected_day"].isoformat():
        return
    selected_date = date.fromisoformat(bundle["day"])
    model = deepcopy(bundle["layout"])
    model["date"] = selected_date
    day_models[selected_date] = model
    panels.clear()
    for panel_id, panel in bundle["panels"].items():
        panels[panel_id] = {"panel_id": panel_id, "panel_type": panel["type"], "state": deepcopy(panel["data"])}
    reconcile_workspace(model)
    project_top_area(True)


def handler_tk():
    operation = top()["op"]
    if operation == "EVENT":
        drop(); push("mem", "APPLY_EVENT"); return
    if operation == "LOAD_DAY":
        drop(); push("mem", "LOAD_DAY"); return
    if operation == "RENDER_DAY":
        render_day_bundle(); drop(); return
...
def handler_mem():
    ...
    if operation == "LOAD_DAY":
        if day_text not in memory["days"]:
            memory["days"][day_text] = make_memory_day(day_text)
            set_register("day-bundle", make_day_bundle(day_text))
            drop(); push("tk", "RENDER_DAY"); push("disk", "WRITE_DAY"); return
        set_register("day-bundle", make_day_bundle(day_text)); drop(); push("tk", "RENDER_DAY"); return
    if operation == "INSTALL_DAY":
        install_day_bundle(day_text, get_register("day-bundle")); set_register("day-bundle", make_day_bundle(day_text)); drop(); push("tk", "RENDER_DAY"); return
    if operation == "APPLY_EVENT":
        changed = apply_memory_event(day_text, get_register("event"))
        if not changed:
            drop()
            return
        set_register("day-bundle", make_day_bundle(day_text)); drop(); push("tk", "RENDER_DAY"); push("disk", "WRITE_DAY"); return
...
def handler_disk():
    ...
    if top()["op"] == "READ_DAY":
        if day_text not in disk_store["days"]:
            raise RuntimeError(f"disk has no persisted day {day_text}")
        set_register("day-bundle", deepcopy(disk_store["days"][day_text])); drop(); return
    if top()["op"] == "WRITE_DAY":
        disk_store["days"][day_text] = deepcopy(get_register("day-bundle")); drop(); return
    raise ValueError(f"unknown Disk operation {top()['op']}")
...
def post_event(event):
    if event["type"] == SECOND_ELAPSED:
        ...
        project_top_area()
        return
    if event["type"] in (SELECT_PREVIOUS_DAY, SELECT_NEXT_DAY, SELECT_TODAY):
        if event["type"] == SELECT_PREVIOUS_DAY:
            g["selected_day"] -= timedelta(days=1)
        elif event["type"] == SELECT_NEXT_DAY:
            g["selected_day"] += timedelta(days=1)
        else:
            g["selected_day"] = date.today()
        begin_stack("tk", "LOAD_DAY")
        set_register("day", g["selected_day"].isoformat())
    else:
        begin_stack("tk", "EVENT")
        set_register("day", g["selected_day"].isoformat())
        set_register("event", deepcopy(event))
    submit_stack()


def start_mobile_stack_workers():
    for name in ("mem", "disk"):
        Thread(target=run_worker_machine, name=name, daemon=True).start()


def request_mobile_stack_shutdown():
    if g["shutting-down"]:
        return
